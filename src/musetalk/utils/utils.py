...
def load_all_model(
    ...
    # Resolve VAE model path
    # If vae_type is a simple name like "sd-vae", construct the local path
    # Otherwise, use it as-is (could be a full path or HuggingFace model ID)
    if "/" not in vae_type and not os.path.isabs(vae_type):
        # It's a simple name, construct the local path
        vae_model_path = os.path.join("models", vae_type)
        # If local path doesn't exist, try HuggingFace model ID as fallback
        if not os.path.exists(vae_model_path):
            if vae_type == "sd-vae":
                # Try HuggingFace model ID as fallback
                vae_model_path = "stabilityai/sd-vae-ft-mse"
    else:
        # It's already a full path or HuggingFace model ID
